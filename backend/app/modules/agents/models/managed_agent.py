...
class AgentTask(Base):
    ...
    requester_agent_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("managed_agents.id"), nullable=True, index=True)
    worker_agent_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("managed_agents.id"), nullable=True, index=True)
    task_description: Mapped[str | None] = mapped_column(Text, nullable=True)
    payment_amount: Mapped[float] = mapped_column(Numeric(18, 4), nullable=False, default=0)
