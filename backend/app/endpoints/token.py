...
def stake_tokens(payload: StakeRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    try:
        WalletService.lock_tokens(db, wallet, payload.amount)
        stake = Stake(
            wallet_id=wallet.id,
...
def unstake_tokens(payload: UnstakeRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if stake.unlock_at > datetime.utcnow():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Stake is still locked")

    WalletService.unlock_tokens(db, wallet, float(stake.amount))
    db.delete(stake)
    db.commit()
